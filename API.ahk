...
; Modified by: Abrucci
; https://github.com/CyxSolutions/AHK-SAMP-API
; Added Functionality for SAMP:CAC Offset
...
global ADDR_CPED_DMGBYWEAPON		:= 0x760		 ;Player Damage by Weapon																 
;
...
; ######################### Additional Adresses by Abrucci #########################
global ADDR_HUD_TITLE						:= 0xBAB240
global ADDR_HUD_WANTED						:= 0xBAB244
global ADDR_HUD_RADIO						:= 0xBAB24C
global ADDR_HUD_WHITE						:= 0xBAB238
global ADDR_HUD_GREEN						:= 0xBAB230
global ADDR_HUD_RED							:= 0xBAB22C
global ADDR_HUD_YELLOW						:= 0xBAB258
...
; #     - AntiPause()								Führt Windows & Cursor Fix ein
; #                                                                                                                             #
; ###############################################################################################################################
...
ChatMessage(message) {
	AddChatMessage("{e84621}[Staatsbinder]{FFFFFF} " message)
}
...
AntiPause() {
	...
	writeMemory(hGTA, ADDR_FIX_WINDOW, 1, 1, "Byte")
	writeMemory(hGTA, ADDR_FIX_CURSOR, 1, 1, "Byte")
    return
}
...
GetSprintInfo(){
    ...
    return Round((Round(sprintInfo) / 3147) * 100)
}
